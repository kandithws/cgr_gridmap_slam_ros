//
// Created by kandithws on 30/6/2560.
//

#ifndef CGR_GRIDMAP_SLAM_ROS_ICP_HPP_H
#define CGR_GRIDMAP_SLAM_ROS_ICP_HPP_H
...
namespace cgr_slam{
typedef GMapping::Point Point;
typedef std::pair<Point,Point> PointPair;

template <typename PointPairContainer>
double CgrScanMatcher::icpStep(GMapping::OrientedPoint & retval, const PointPairContainer& container){
  ...
  //LOGPRINT_INFO("ICPSTEP: mean.first=%lf, mean.second=%lf", mean.first, mean.second);
  ...
  //LOGPRINT_INFO("ICPSTEP: sxx=%lf, sxy=%lf, syx=%lf, syy=%lf", sxx,sxy,syx,syy);
  ...
  //LOGPRINT_INFO("ICPSTEP: Return Pose=(%lf, %lf, %lf)", retval.x, retval.y, retval.theta);
...
template <typename PointPairContainer>
double CgrScanMatcher::icpNonlinearStep(GMapping::OrientedPoint & retval, const PointPairContainer& container){
  typedef typename PointPairContainer::const_iterator ContainerIterator;
  PointPair mean=std::make_pair(Point(0.,0.), Point(0.,0.));
  int size=0;
  for (ContainerIterator it=container.begin(); it!=container.end(); it++){
...
#endif //CGR_GRIDMAP_SLAM_ROS_ICP_HPP_H
